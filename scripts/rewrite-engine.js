...
const file = path.join(__dirname, '..', 'lib', 'swarm', 'engine.ts');
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  const executor = new GraphExecutor(graph, {
    onRoundStart: async (round) => {
      ...
      const fs = swarmStore.getState().features;
    ...
    executeNode: async (node, context, round) => {
      ...
      const evaluatorHistorySuffix = prevFeedback
        ? \`\\n\\n--- PREVIOUS EVALUATOR FEEDBACK ---\\n\${maybeCompress(prevFeedback, fs.contextCompression)}\`
        : "";
      const batchArtifactSuffix = batchArtifacts.text
        ? \`\\n\\n--- BATCH ARTIFACT CONTEXT ---\\n\${maybeCompress(batchArtifacts.text, fs.contextCompression)}\`
        : "";
      const researchSuffix = context["research"]?.text
        ? \`\\n\\n--- RESEARCH CONTEXT ---\\n\${maybeCompress(context["research"].text, fs.contextCompression)}\`
        : "";
      ...
      if (node.id === "worker1") {
        ...
        const before = fs.heuristicSelector || fs.lintLoop ? await collectFingerprints(opts.workspace) : new Map();
        ...
        const after = fs.heuristicSelector || fs.lintLoop ? await collectFingerprints(opts.workspace) : new Map();
      ...
      if (node.id === "evaluator") {
        ...
        const evaluator = await runAgentTask({
          ...
          prompt: \`\${baseEval}\${evaluatorHistorySuffix}\${researchSuffix}\${batchArtifactSuffix}\${formatDirectiveBlock(
            ...
            carryNextActions,
          )}\\n\\n--- WORKER-1 OUTPUT ---\\n\${maybeCompress(worker1?.text || "", fs.contextCompression)}\`,
      ...
      if (node.id === "worker2") {
        ...
        if (fs.heuristicSelector && changedFiles.length === 0) {
          worker2Skipped = true;
      ...
      if (node.id === "coordinator") {
        ...
        const coordinatorPrompt = \`\${baseCoord}\\n\\nRound: \${round}\\nWorkspace: \${opts.workspace}\\nLint: ran=\${lint.ran}; passed=\${lint.passed}; command=\${lint.command}; exit=\${lint.exitCode}\\nChanged files (\${changedFiles.length}):\\n\${changedFiles.length ? changedFiles.join("\\n") : "(none)"}\\n\\n\${continuityBlock}\\n\\nResearch:\\n\${context["research"] ? maybeCompress(context["research"].text, fs.contextCompression) : "(disabled)"}\\n\\nBatch Artifact Context:\\n\${batchArtifacts.text ? maybeCompress(batchArtifacts.text, fs.contextCompression) : "(none)"}\\n\\nWorker-1 Output:\\n\${maybeCompress(worker1?.text || "", fs.contextCompression)}\\n\\nWorker-2 Output:\\n\${maybeCompress(worker2?.text || "", fs.contextCompression)}\\n\\nEvaluator Output:\\n\${maybeCompress(evaluator?.text || "", fs.contextCompression)}\\n\`;

        const coordinator = fs.ensembleVoting
    ...
    onRoundEnd: async (round) => {
      // Just returning true makes it continue...
      return true;
    }
  });

  // Keep references to node outputs
  ...
  executor["config"].onRoundEnd = async (round) => {
    ...
    const shouldRewind = fs.checkpointing && round > 1 && (coordStatus === "FAIL" || (!lint.passed && (defects.high > 0 || defects.med > 0)));
    ...
    if (shouldRewind) {
      ...
      if (fs.humanInLoop || fs.checkpointing) {
