...
A multi-agent orchestrator with a **dual-runtime architecture** — one Next.js dashboard, switchable backend:

| Runtime | Backend | Set via |
|---------|---------|---------|
| **custom** (default) | Built-in swarm engine (`lib/swarm/engine.ts`) | `SWARM_RUNTIME=custom` |
| **adk** | Google ADK / Vertex AI Python sidecar (`adk_prototype/`) | `SWARM_RUNTIME=adk` |

Both runtimes are accessed through a unified **runtime adapter** (`lib/swarm/runtime.ts`) — the dashboard, API routes, and CLI all call the same interface regardless of which backend is active.

Additional optional sidecar: Microsoft Agent Framework workflow in `foundry_agents/workflow_server.py`.
...
- Custom runtime: full orchestrator with pause/resume/rewind, stale recovery, concurrency guard
- ADK runtime: HTTP client adapter (`runtime-adk.ts`) ready — connects to a Python sidecar at `ADK_SIDECAR_URL` (default `http://127.0.0.1:8420`)
- Dashboard and CLI read the same persisted swarm state
- Pause, resume, and rewind work from API, CLI, or dashboard
- Runtime adapter pattern: all consumers go through `getRuntime()` — switching backends requires only changing `SWARM_RUNTIME`
